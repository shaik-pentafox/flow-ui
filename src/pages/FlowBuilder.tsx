...
const createNode = ({ data, position, actions }: CreateNodeParams): FlowNode => ({
  id: nanoid(),
  type: "custom",
  position,
  data: {
...
const initialEdges: Edge[] = [{ id: "n4-n5", source: "n4", target: "n5" }];
...
export function FlowBuilder() {
  ...
  const addNode = useCallback(
    (data: FeatureData, position: { x: number; y: number } = { x: 100, y: 100 }) => {
      setNodes((nds) => [
        ...nds,
        createNode({
          data,
        ...
        <main className="flex flex-col flex-1">
          <div className="flex flex-col m-2 ml-0">
            <FlowHeader
              ...
              onSubmit={(data) => {
                console.log("Submitted Flow Header:", data);
              }}
            />
          </div>
